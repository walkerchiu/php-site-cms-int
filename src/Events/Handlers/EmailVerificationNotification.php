...
class EmailVerificationNotification extends Notification
{
    ...
    {
        $link = config('wk-site-cms.client.link.backend') .'/'. config('wk-site-cms.client.link.password-reset');
        if ($event->admin)
            $link = config('wk-site-cms.client.url') .'/'. $link;

        switch (config('wk-site-cms.client.mode')) {
            case 'lighthouse-graphql-passport':
                $link = config('wk-site-cms.client.url') .'/'. $link .'?token='. $event->token;
                break;
            default:
