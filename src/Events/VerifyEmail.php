...
class VerifyEmail extends BaseClass
{
    ...
    {
        $this->user = $user;
        $this->link = $this->verificationUrl($user, config('wk-site-cms.client.mode'));
    ...
    protected function verificationUrl($notifiable, $client = null)
    {
        if (empty($mode)) {
            ...
            if (empty(config('lighthouse-graphql-passport.verify_email.base_url')))
                return config('wk-site-cms.client.url') .'/'. config('wk-site-cms.client.link.email-verify') .'?token='. $payload;

            return config('lighthouse-graphql-passport.verify_email.base_url') .'?token='. $payload;
        } else {
            return config('wk-site-cms.client.url') .'/'. config('wk-site-cms.client.link.email-verify') .'?token='. sha1($notifiable->getEmailForVerification());
        }
    }
