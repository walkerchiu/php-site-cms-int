...
namespace WalkerChiu\SiteCMS\Models\Repositories;
...
class LayoutRepositoryWithImage extends Repository
{
    use FormTrait;
    use RepositoryTrait;
    ...
    {
        $this->instance = App::make(config('wk-core.class.site-cms.layout'));
    }
    ...
    public function list(string $code, array $data, $is_enabled = null, $auto_packing = false)
    {
        ...
        if ($auto_packing) {
            $factory = new PackagingFactory(config('wk-site-cms.output_format'), config('wk-site-cms.pagination.pageName'), config('wk-site-cms.pagination.perPage'));
            $factory->setFieldsLang(['name', 'description', 'keywords', 'content']);

            if (in_array(config('wk-site-cms.output_format'), ['array', 'array_pagination'])) {
                switch (config('wk-site-cms.output_format')) {
                    case "array":
    ...
    public function show($instance, $code): array
    {
        $data = [
            ...
            'constant' => [
                'layoutType' => config('wk-core.class.site-cms.layoutType')::options(true)
            ],
            'basic'    => [],
        ...
        if (is_string($code)) {
            $data['basic'] = [
                ...
                'type'           => $instance->type,
                'type_text'      => trans('php-site-cms::layout.layoutType.'. $instance->type),
        ...
        if (config('wk-site-cms.onoff.morph-comment'))
